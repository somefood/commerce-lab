...
자기 판단을 말한다. 합의 결과는 Claude가 ADR로 남긴다 (§4-1).
...
## 4-1. ADR 작성 규칙 (2026-08-24 변경)

**원래는 사용자가 직접 썼다.** "자기 언어로 옮겨 적는 것이 이해했다는 증거"라는 이유였다.
이 프로젝트의 목적이 전체 흐름 파악으로 옮겨가면서 Claude가 쓰는 것으로 바꿨다.
사용자는 읽고 어색한 부분을 고친다 — 그것만으로도 원래 목적의 절반은 남는다.

절 네 개를 지킨다. **대안과 결과가 ADR의 값어치다.**

| 절 | 쓰는 것 |
|---|---|
| 배경 | 결정하기 **전** 상황. 해결책을 미리 쓰지 않는다 |
| 결정 | 무엇을 하기로 했나. 한 문장으로 시작 |
| 대안 | **버린 것과 버린 이유** |
| 결과 | 얻은 것 **과 포기한 것**. 같은 무게로 |

### 문체

**2~3년차 주니어 개발자가 읽고 이해할 수 있어야 한다.**

- 장황하게 쓰지 않는다. 한 ADR은 스크롤 두세 번 안에 끝난다
- 용어를 처음 쓸 때 한 줄로 풀어준다 (`대사(reconciliation) — 두 곳의 값을 맞춰보는 것`)
- 추상적인 설명보다 **실제로 관측한 숫자와 증상**을 쓴다
  ("롤백이 안 됐다"보다 "성공 1건인데 orders 테이블에 2행이 남았다")
- 코드는 필요할 때만, 5줄 이내로
- 자랑하지 않는다. 포기한 것을 빼먹으면 ADR이 아니라 홍보문이다
